...
//     mkdir(path.join(createdTmpDir), { recursive: true }, () => {}),
//     mkdir(path.join(createdTmpDir, 'images'), { recursive: true }, () => {}),
//     mkdir(path.join(createdTmpDir, 'excalidraw'), { recursive: true }, () => {}),
// ]);
...
//         let res: IpcResponse<void> = await handler(ipcMainInvokeEvent, { image: { ...fakeCatalogImage, name: newName } });
//         expect(res.success).toBe(true);
//     });
